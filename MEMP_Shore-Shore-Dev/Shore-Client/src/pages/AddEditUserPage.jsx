...
const AddEditUserPage = () => {
    ...
    const [user, setUser] = useState({
        ...
        fleet: '',   
        vessels: '', 
        isActive: true, // Added IsActive state
        imageUrl: null,
        userRights: FALLBACK_USER_RIGHTS_OPTIONS[2].value,
        require2FA: false, // NEW: Added 2FA toggle
    });
    ...
    const handleDeleteUser = async () => {
        ...
        try {
            ...
            // UPDATED: Redirect to MEMP Admin Users
    ...
    // --- FILTERING LOGIC ---
    // 1. Parse selected Fleet names from the comma-separated string
    const selectedFleetNames = user.fleet ? user.fleet.split(',').map(s => s.trim()) : [];

    // 2. Filter vessels: 
    //    - Keep only if vessel's fleetName is in selectedFleetNames.
    //    - Handles both 'fleetName' and 'FleetName' property cases safely.
    const filteredVesselOptions = vesselOptions.filter(v => {
        const vFleet = v.fleetName || v.FleetName; // Handle casing variations
        return vFleet && selectedFleetNames.includes(vFleet);
    });
                ...
                <div className="right-column">
                    <div className="details-card card-shadow">
                            ...
                            <div className="input-group">
                                <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                                    <input 
                                        ...
                                        onChange={(e) => setUser({ ...user, require2FA: e.target.checked })}
                                        disabled={isViewMode}
                                        style={{ width: 'auto', cursor: 'pointer' }}
                                    />
                                    <span>Require Two-Factor Authentication (OTP via Email)</span>
                                </label>
                                <small style={{ color: '#666', marginTop: '5px', display: 'block' }}>
                                    When enabled, user will receive OTP via email during login
                                </small>
                            </div>
                        ...
                        <div className="selection-area">
                            ...
                            {/* Updated: Pass filteredVesselOptions instead of all vesselOptions */}
                            <CheckboxGroup 
                                label="Assigned Vessels" 
                                options={filteredVesselOptions} 
                                fieldName="vessels" 
                        ...
                        <div className="card-footer">
                            ...
                            {isViewMode && (
                                // UPDATED: Redirect to correct MEMP Edit route
